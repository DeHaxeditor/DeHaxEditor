import { json,requireProfile,errResponse,sb,mpSubscriptionsToken } from './_lib.mjs';
...
export const handler=async event=>{
  ...
  try{
    ...
    const {data:localPayments}=await sb(`/rest/v1/payment_orders?user_id=eq.${encodeURIComponent(user.id)}&select=id,provider_order_id,kind,status,amount,access_days,plan_code,payment_method,access_granted_at,created_at&order=created_at.desc&limit=30`);
    ...
    const payments=[...(localPayments||[]),...recurringInvoices]
      ...
      .slice(0,30);
    return json(200,{profile:{id:p.id,displayName:p.display_name||'',email:user.email||p.email||'',plan:p.plan,subscriptionStatus:p.subscription_status,accessExpiresAt:p.access_expires_at,proStartedAt:p.pro_started_at,createdAt:p.created_at},subscription,payments});
