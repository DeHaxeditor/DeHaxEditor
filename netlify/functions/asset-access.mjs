...
export default async req=>{
  ...
  try{
    ...
    if(!assetId||!['download','preview'].includes(action))return asResponse(json(400,{error:'Solicitação inválida.'}));
    ...
    if(action==='preview'&&!a.preview_enabled)throw Object.assign(new Error('O preview deste item está temporariamente pausado.'),{status:423});
    ...
    const key=action==='preview'?(f?.preview_key||((a.media_type==='audio'||a.media_type==='video'||a.media_type==='image')?f?.file_key:null)):f?.file_key;
    if(!key)throw Object.assign(new Error(action==='preview'?'Este item ainda não possui preview.':'Arquivo ainda não foi enviado.'),{status:404});
