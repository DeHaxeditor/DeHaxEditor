(() => {
  ...
  async function accountOverview(){if(demoEnabled&&!config.supabaseUrl)return {profile:{displayName:'Editor Demo',email:'demo@dehax.local',plan:'pro',subscriptionStatus:'authorized',createdAt:new Date().toISOString()},subscription:{status:'authorized',nextPaymentDate:new Date(Date.now()+20*86400000).toISOString(),amount:19.9,currency:'BRL'},payments:[]};return callFunction('account-overview',{})}
