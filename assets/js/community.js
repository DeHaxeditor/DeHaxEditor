(() => {
  ...
  async function loadSettings(){
    ...
    const monthly=Number(String(s.pro_monthly_price||s.pro_price||'19.90').replace(',','.'))||19.9, semTotal=Number(String(s.pro_semester_total||'59.40').replace(',','.'))||59.4, semEq=Number(String(s.pro_semester_monthly_equiv||'9.90').replace(',','.'))||9.9, pixDays=Math.max(1,Number(s.pix_access_days||30)||30), saving=Math.max(0,monthly*6-semTotal); if($('#proMonthlyPrice'))$('#proMonthlyPrice').textContent=monthly.toLocaleString('pt-BR',{minimumFractionDigits:2});if($('#proSemesterMonthly'))$('#proSemesterMonthly').textContent=semEq.toLocaleString('pt-BR',{minimumFractionDigits:2});if($('#proPixDays'))$('#proPixDays').textContent=String(Math.round(pixDays));if($('#proSaving'))$('#proSaving').textContent=`6 meses por R$ ${semTotal.toLocaleString('pt-BR',{minimumFractionDigits:2})} à vista · economia de R$ ${saving.toLocaleString('pt-BR',{minimumFractionDigits:2})}`;
